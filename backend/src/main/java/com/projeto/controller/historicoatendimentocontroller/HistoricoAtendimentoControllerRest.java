...
public class HistoricoAtendimentoControllerRest extends RootController implements IHistoricoAtendimentoControllerRest {
    ...
    public void handle(HttpExchange exchange) throws IOException {
        ...
        if(method.equals(HttpMethod.GET.getMethod()) && path.equals(ApiRotas.HISTORICO_ATENDIMENTO_FIND_ALL.getPath())){
            findAll(exchange);
        } else if (method.equals(HttpMethod.GET.getMethod()) && path.matches(ApiRotas.HISTORICO_ATENDIMENTO_FIND_BY_ID.getPath())) {
            findById(exchange);
        } else if (method.equals(HttpMethod.POST.getMethod()) && path.equals(ApiRotas.HISTORICO_ATENDIMENTO_CREATE.getPath())) {
            save(exchange);
        } else if (method.equals(HttpMethod.PUT.getMethod()) && path.matches(ApiRotas.HISTORICO_ATENDIMENTO_UPDATE.getPath())) {
            update(exchange);
        } else if (method.equals(HttpMethod.DELETE.getMethod()) && path.matches(ApiRotas.HISTORICO_ATENDIMENTO_DELETE.getPath())) {
            delete(exchange);
        } else {
    ...
    public void findById(HttpExchange exchange) throws IOException {
        ...
        HistoricoAtendimento historicoAtendimento = historicoAtendimentoService.buscarHistoricoAtendimentoPorId(id);
        if (historicoAtendimento == null) {
            sendResponse(exchange, "Historico de atendimento não encontrado", 404);
            return;
        }

        String response = gson.toJson(historicoAtendimento);
        exchange.getResponseHeaders().set("Content-Type", "application/json");
        sendResponse(exchange, response, 200);
    }
    ...
    public void update(HttpExchange exchange) throws IOException {
        ...
        HistoricoAtendimento historicoAtendimento = gson.fromJson(body, HistoricoAtendimento.class);
        historicoAtendimento.setIdHistoricoAtendimento(id);
        historicoAtendimentoService.atualizarHistoricoAtendimento(historicoAtendimento);

        sendResponse(exchange, "Historico de atendimento atualizado com sucesso", 200);
    }

    @Override
    public void delete(HttpExchange exchange) throws IOException {
        Long id = extractIdFromPath(exchange.getRequestURI().getPath());
        historicoAtendimentoService.removerHistoricoAtendimento(id);
        sendResponse(exchange, "Historico de atendimento deletado com sucesso", 200);
