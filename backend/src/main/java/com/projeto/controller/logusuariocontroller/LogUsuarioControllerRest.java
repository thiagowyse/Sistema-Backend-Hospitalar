...
public class LogUsuarioControllerRest extends RootController implements ILogUsuarioControllerRest {
    ...
    public void handle(HttpExchange exchange) throws IOException {
        ...
        if(method.equals(HttpMethod.GET.getMethod()) && path.equals(ApiRotas.LOG_USUARIO_FIND_ALL.getPath())){
            findAll(exchange);
        } else if (method.equals(HttpMethod.GET.getMethod()) && path.matches(ApiRotas.LOG_USUARIO_FIND_BY_ID.getPath())) {
            findById(exchange);
        } else if (method.equals(HttpMethod.POST.getMethod()) && path.equals(ApiRotas.LOG_USUARIO_CREATE.getPath())) {
            save(exchange);
        } else if (method.equals(HttpMethod.PUT.getMethod()) && path.matches(ApiRotas.LOG_USUARIO_UPDATE.getPath())) {
            update(exchange);
        } else if (method.equals(HttpMethod.DELETE.getMethod()) && path.matches(ApiRotas.LOG_USUARIO_DELETE.getPath())) {
            delete(exchange);
        } else {
    ...
    public void findById(HttpExchange exchange) throws IOException {
        ...
        LogUsuario logUsuario = logUsuarioService.buscarLogUsuarioPorId(id);
        if (logUsuario == null) {
            sendResponse(exchange, "Log de usuario não encontrado", 404);
            return;
        }

        String response = gson.toJson(logUsuario);
        exchange.getResponseHeaders().set("Content-Type", "application/json");
        sendResponse(exchange, response, 200);
    }
    ...
    public void update(HttpExchange exchange) throws IOException {
        ...
        LogUsuario logUsuario = gson.fromJson(body, LogUsuario.class);
        logUsuario.setIdLogUsuario(id);
        logUsuarioService.atualizarLogUsuario(logUsuario);

        sendResponse(exchange, "Log de usuario atualizado com sucesso", 200);
    }

    @Override
    public void delete(HttpExchange exchange) throws IOException {
        Long id = extractIdFromPath(exchange.getRequestURI().getPath());
        logUsuarioService.removerLogUsuario(id);
        sendResponse(exchange, "Log de usuario deletado com sucesso", 200);
