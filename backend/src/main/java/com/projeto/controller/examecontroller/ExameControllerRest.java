...
public class ExameControllerRest extends RootController implements IExameControllerRest {
    ...
    public void handle(HttpExchange exchange) throws IOException {
        ...
        if(method.equals(HttpMethod.GET.getMethod()) && path.equals(ApiRotas.EXAME_FIND_ALL.getPath())){
            findAll(exchange);
        } else if (method.equals(HttpMethod.GET.getMethod()) && path.matches(ApiRotas.EXAME_FIND_BY_ID.getPath())) {
            findById(exchange);
        } else if (method.equals(HttpMethod.POST.getMethod()) && path.equals(ApiRotas.EXAME_CREATE.getPath())) {
            save(exchange);
        } else if (method.equals(HttpMethod.PUT.getMethod()) && path.matches(ApiRotas.EXAME_UPDATE.getPath())) {
            update(exchange);
        } else if (method.equals(HttpMethod.DELETE.getMethod()) && path.matches(ApiRotas.EXAME_DELETE.getPath())) {
            delete(exchange);
        } else {
    ...
    public void findById(HttpExchange exchange) throws IOException {
        ...
        if (exame == null) {
            sendResponse(exchange, "Exame não encontrado", 404);
            return;
        }

        String response = gson.toJson(exame);
        exchange.getResponseHeaders().set("Content-Type", "application/json");
        sendResponse(exchange, response, 200);
    }
    ...
    public void update(HttpExchange exchange) throws IOException {
        ...
        Exame exame = gson.fromJson(body, Exame.class);
        exame.setIdExame(id);
        exameService.atualizarExame(exame);

        sendResponse(exchange, "Exame atualizado com sucesso", 200);
    }
    ...
    public void delete(HttpExchange exchange) throws IOException {
        ...
        sendResponse(exchange, "Exame deletado com sucesso", 200);
